...
from menu_ru_data import COVER, PAGES, ALLERGENS
...
def render_content_page(blocks, page_label):
    ...
    # them off with no visible error), which shipped undetected in the
    # first RU/DE/FR/PL PDFs. Do not loosen these without re-running that
    # per-page overflow check for every language's data file.
...
def render_allergen_page():
    ...
    d.text((x + en_w, y + 4), "Аллергены", font=f_h1_ru, fill=RED)
    y += 60
...
def load_photo_page(num):
    ...
    ru = "Названия блюд и цены — на предыдущей странице"
...
def main():
    ...
    out = os.path.join(REPO_DIR, "assets", "menu-ru.pdf")
